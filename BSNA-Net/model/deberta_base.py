...
class BSNADeBERTaBase(BSNAClassifierBase):
    def __init__(self, num_classes, num_subsets, z_dim=256, pretrained=False, model_name="microsoft/deberta-base"):
        super().__init__(num_subsets, z_dim)
        if pretrained:
            self.backbone = DebertaForSequenceClassification.from_pretrained(model_name, num_labels=num_classes)
        else:
            config = DebertaConfig(num_labels=num_classes)
            self.backbone = DebertaForSequenceClassification(config)
    ...
    def encode_input(self, input_ids, attention_mask=None):
        e = self.backbone.deberta.embeddings.word_embeddings(input_ids)
